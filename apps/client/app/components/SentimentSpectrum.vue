  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex justify-between items-center mb-1">
        ...
        <span :class="['text-xs font-bold', getSentimentColor(article.sentimentScore)]">
          {{ getSentimentLabel(article.sentimentScore) }} ({{ article.sentimentScore.toFixed(2) }})
        </span>
      </div>

      <!-- Spectrum Bar -->
      <div class="h-3 w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden relative">
        <!-- Center Pointer -->
        <div class="absolute left-1/2 top-0 bottom-0 w-0.5 bg-neutral-300 dark:bg-neutral-600 z-10"></div>

        <!-- Value Bar -->
        <div
          class="absolute top-0 bottom-0 h-full transition-all duration-1000 ease-out"
          :class="[article.sentimentScore > 0 ? 'bg-green-500/50' : 'bg-red-500/50']"
          :style="{
            left: article.sentimentScore > 0 ? '50%' : `${50 + (article.sentimentScore * 50)}%`,
